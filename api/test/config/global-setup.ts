...
function waitForPostgres() {
    for (let i = 0; i < 20; i++) {
        try {
            execSync(`docker exec skillfusion_db_test pg_isready -U ${process.env.POSTGRES_USER}`, {
                stdio: 'ignore',
            });
            return;
        } catch {
...
before(() => {
    ...
    execSync(
        `docker run -d --name skillfusion_db_test -p 5434:5432 -e POSTGRES_USER=${process.env.POSTGRES_USER} -e POSTGRES_PASSWORD=${process.env.POSTGRES_PASSWORD} -e POSTGRES_DB=${process.env.POSTGRES_DB} postgres:18`,
        { stdio: 'inherit' }
    );
